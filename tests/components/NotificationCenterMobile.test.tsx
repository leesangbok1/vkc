...
describe('NotificationCenterMobile', () => {
  ...
  it('marks all notifications as read and clears unread badge', async () => {
    render(<NotificationCenterMobile />)
    ...
    await screen.findByText(notificationsFixture[0].title)

    const [markAllButton] = await screen.findAllByRole('button', { name: /모두 읽음$/ })
    fireEvent.click(markAllButton)

    expect(mockedNotificationService.markAllAsRead).toHaveBeenCalledWith('user-test')
